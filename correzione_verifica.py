        ...
        for chef in concorso_cucina.keys():
            ...
            for categoria_piatto, voti, categoria_chef in concorso_cucina[chef]:
                if(categoria_piatto==piatto and categoria_chef==tipo_chef ):
                    ...
                    tot+=voti[2]
            if(tot==max):
                lista_chef.append(chef)
        return [max,lista_chef]
    else:
        return 0
...
if(totale_massimo(concorso_cucina, piatto, tipo_chef)[0]==0):
    print("ERRORE, impossibile calcolare il totale! Piatto e/o chef non presenti!")
else:
    print(f"Votazione massima: {totale_massimo(concorso_cucina, piatto, tipo_chef)[0]}, ottenuta dallo/dagli chef {totale_massimo(concorso_cucina, piatto, tipo_chef)[1]}")
...
-richiesta 6 A: Ho fatto in modo che la funzione totale_massimo ritorni una lista con questa struttura: [max,lista_chef]. A differenza di 
prima verrà ritornata una lista e non un'unica stringa per lo chef che ha ottenuto la valutazione più alta.
-richiesta 7: 
    -nella funzione inserisci_dati_nuovo_chef() ho migliorato la validazione dell'input delle 3 valutazioni, controllando che il 
    voto sia anche minore uguale a 10 oltre che maggiore di 0.
    -sempre nella stessa funzione ho aggiunto una parentesi nel metodo append, in modo che venisse aggiunta una tupla alla lista e non solo
